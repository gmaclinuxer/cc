...
!function(){
    ...
    */
    var treeContainerChartObj = {
        ...
        template:function(data){
          var node=data.item;
          //return node.Name+'</span><span class="host-count label">'+node.number+'</span><span class="host-comefrom label label-success">阿里云</span>';
        ...
        select:function(e){
            ...
            /*点击显示模块属性
            var data = {'application' : {'ApplicationID' : node.id}, 'set' : {'ApplicationID' : node.appId, 'SetID' : node.id}, 'module' : {'ApplicationID' : node.appId, 'ModuleID' : node.id}};
            var url = {'application' : '/app/getAppById', 'set' : '/set/getSetById', 'module' : '/module/getModuleById'};
    ...
    */
    $('#filter_module_mine').click(function(e){
        ...
        //现改为搜索维护人为当前用户的主机
        gridObj.dataSource.transport.read.data = {'ApplicationID' : cookie.get('defaultAppId'), 'Operator' : cookie.get('uin').replace('o', '')};
    ...
    */
        ...
        events:function(){
            ...
            hostlist.element.on('change.kendoGrid', 'input[type=checkbox]', function(e){
                ...
                if($(target).parent('th').length>0){//表头
                    ...
                    //hostlist.tbody.find('input[type=checkbox]').prop('checked', checked);
            ...
            /*点击某一行*/
            /*hostlist.element.on('click.kendoGrid', 'tr', function(e){
                var notThead = $(e.target).closest('thead').length==0;
                var notcheckBox = e.target.nodeName.toLowerCase()!='input';
                var notInnerIP = e.target.nodeName.toLowerCase()!='a';

                if(notThead && notcheckBox && notInnerIP){
            ...
            hostlist.element.on('click.kendoGrid','.k-grid-saveAsExcel',function(e){
             ...
             //   $('.c-dialogimg-loading').prop('src','/static/img/loading_2_24x24.gif');
            });
            ...
            hostlist.element.on('click.kendoGrid', '#batRes', function (e){
                ...
                var gridBatRes = dialog({
                    title:'确认',
                    width:300,
                    content: '确认是否将已勾选的<i class="redFont">'+hostId.length+'</i>台主机上交至快速分配池',
                    okValue:"确定",
                    cancelValue:"取消",
                    ok:function (){
                        var d = dialog({
    ...
    var wH=$('.content-wrapper').height(),
        ...
        allColumns=[
            ...
            {field:"Model",title:'型号',filterable:true},
            {field:"Cpu",title:'Cpu',filterable:true},
            {field:"Memory",title:'内存',filterable:true},
            {field:'InnerIP',title:"内网IP",filterable:true},
            {field:'InnerGate',title:"内网网关",filterable:true},
            {field:'InnerInterface',title:"内网网卡",filterable:true},
            {field:'BgpIP',title:"BGP IP",filterable:true},
            {field:'BgpGate',title:"BGP网关",filterable:true},
            {field:'BgpInterface',title:"Bgp网卡",filterable:true},
            {field:"OuterIP", title: "公网IP",filterable:true},
            {field:'OuterGate',title:"公网网关",filterable:true},
            {field:'OuterInterface',title:"公网网卡",filterable:true},
    ...
    gridObj={
            change:function(e){
                ...
                var tdwidth=(grid.select().width());
                userSelection = rowText;
                if(userSelection!=null){
                    var relativeX = (pos.left-545+tdwidth);
                    var relativeY = (pos.top-45);
    ...
    $('#batEdit_window').on('click', '.select-area', function(e){
        ...
        if($(target).attr('data')=='selectAll'){//全选checkbox
            ...
            //$('#batEdit_window select').prop('disabled', !checked);
  ...
  alert("保存失败;"+XmlHttpRequest.responseText);
    ...
    */
    $('#host_query_reset').click(function(){
        ...
        $('#HostName').val('');
        $('#SN').val('');
        // $("input[name='AutoRenew']").prop('checked', false);
        $('#Operator').select2('val','');
        $('#BakOperator').select2('val','');
        $('#CreateTime').val('');
        $('#DeadLineTime').val('');
        $('#DeviceClass').select2('val','');
        $('#OSName').select2('val','');
        $('#Region').select2('val','');
        $('#Source').select2('val','');
        $('#Status').select2('val','');
    ...
    */
    $('#host_query_submit').click(function(){
        ...
        if(AssetID){
            param['AssetID'] = AssetID.replace(/\s+/g, ',').replace(/[\s,]+$/g, '');
        }
        ...
        if(HostName){
            param['HostName'] = HostName.replace(/\s+/g, ',').replace(/[\s,]+$/g, '');
        }

        var SN = $('#SN').val();
        if(SN){
            param['SN'] = SN.replace(/\s+/g, ',').replace(/[\s,]+$/g, '');
        }

        // var AutoRenew = $("[name='AutoRenew']").prop('checked');
        // if(AutoRenew){
...
(function($) {
    $.fn.selectDialog=function(options) {
        ...
        var creat_content=function(data) {
            ...
            var content=toolbar+'<div class="btn-toolbar" id='+ contentId +' style="width:800px;">';
